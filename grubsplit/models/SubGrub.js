...
subGrubSchema.statics.createNewSubGrub = function(userID, grubID, callback) {
  Grub.findOne({
    _id: grubID
  }).populate(['owner']).exec(function(err, grub) {
    if (err) {
      callback({
        msg: 'could not find Grub'
      });
    } else {
      ...
      SubGrub.create({
        ...
        if (err) {
          ...
          grub.save(function(err) {
            if (err) {
              callback({
                msg: 'could not add subgrub id to grub'
              });
            } else {
...
subGrubSchema.statics.getSubGrub = function(subGrubID, callback) {
  SubGrub.findOne({
    _id: subGrubID
  }).populate(['owner', 'grubID']).exec(function(err, subGrub) {
    if (subGrub) {
      callback(null, subGrub);
    } else {
      callback({
        msg: 'could not find subGrub'
      });
    }
  });
}
  ...
  SubGrub.remove({
    ...
    if (err) {
      ...
      Grub.findOneAndUpdate({
        ...
        if (err) {
          callback({
            msg: 'could not delete subgrub ref from grub'
          });
        } else {
          callback(null);
        }
